...
function toolChild(sessionId: string, pair: PairWork, assistantNodeId: string, call: NormalizedToolCall, result: NormalizedToolResult): ChildBuild | undefined {
	...
	const node: SessionContextNode = {
		...
		createdAt: result.lifecycleEndedAt ?? eventTimestamp(pair.prompt), sourceMessageId: result.sourceEntryId, sourceTurnIndex: result.sourceLine, population: "assistant", nodeRole: "child", origin: "tool", childType, pairId: pair.pairId, parentNodeId: assistantNodeId, operation, ...(detail ? { operationDetail: detail } : {}), ...purpose, status: failure.failed ? "failed" : "completed",
