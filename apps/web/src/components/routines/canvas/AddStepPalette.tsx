	...
	return (
		...
		>
			<button
				...
				aria-expanded={open}
				aria-haspopup="menu"
				aria-label={t("routines.addStepPalette.addStepAria")}
			...
			>
				...
				{t("routines.addStepPalette.addStep")}
				<ChevronDown
			...
			{open ? (
				<div
					role="menu"
					aria-label={t("routines.addStepPalette.menuAria")}
						...
						>
							<div className="bg-paper-2/60 px-2.5 pb-1 pt-1.5">
								<div className="font-mono text-2xs uppercase tracking-meta text-ink-3">
									{t(`routines.stepPalette.categories.${cat.key}.label`)}
								</div>
								<div className="text-2xs text-ink-4">
									{t(`routines.stepPalette.categories.${cat.key}.tagline`)}
								</div>
							</div>
							<div className="py-1">
								{cat.entries.map((entry) => (
									<button
										...
										aria-label={t("routines.addStepPalette.addEntryAria", {
											label: t(`routines.stepDescriptors.${entry.key}.label`),
										})}
									...
									>
										<span className="shrink-0 pt-px font-mono text-2xs uppercase tracking-meta text-accent">
											{t(`routines.stepDescriptors.${entry.key}.label`)}
										</span>
										<span className="text-2xs leading-snug text-ink-2">
											{t(`routines.stepDescriptors.${entry.key}.help`)}
										</span>
									</button>
								))}
							</div>
